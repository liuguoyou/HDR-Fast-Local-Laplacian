...
%% fast llf version
function R = lapfilter_core(I,r, ref_num)
...
%create ref
discretisation=linspace(min(min(I)),max(max(I)),ref_num);
discretisation_step=discretisation(2)-discretisation(1);
...
for ref=discretisation
    ...
    for level=1:n_levels-1
        L{level} = L{level} + (abs(G{level} - ref) < discretisation_step) .* temp_laplace_pyr{level} .* (1 - abs(G{level} - ref) / discretisation_step);
    end
end
L{end} = G{end};  % residual not affected
R = reconstruct_laplacian_pyramid(L);  % collapse result Laplacian pyramid
